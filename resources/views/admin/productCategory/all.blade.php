...
@section('content_admin')
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div class="x_title">
                <h2>دسته بندی های محصول</h2>
                <ul class="nav navbar-right panel_toolbox">
                    <a href="{{route('productCategory.create')}}" class="btn btn_admin_top"><i class="fa fa-plus"></i> ثبت دسته بندی</a>
                ...
                <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap text-center" cellspacing="0" width="100%">
                    ...
                    @foreach($all as $key => $item)
                        <tr class="odd gradeX text-center" id="course{{$item->id}}">
                            ...
                            <td>@if($item->status == '0')غیرفعال@elseif($item->status == '1')فعال@endif</td>

                            <td>
                                <a href="{{route('productCategory.edit', $item->id)}}" class="btn btn_admin" title="ویرایش اطلاعات"><i class="fa fa-edit"></i></a>
                                ...
                                <form id="deleteRecordForm_{{$item->id}}"
                                      action="{{route('productCategory.destroy', $item->id)}}"
