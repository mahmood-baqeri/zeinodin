...
@section('ajax')
    <script>
        function checkDuplicateCourseDiscountCode() {
            var form_data = new FormData();
            form_data.append('code', $('.courseDiscountCode').val());
            $.ajax({
                url: "/api/checkDuplicateCourseDiscountCode",
                type: "post",
                data: form_data,
                contentType: false,
                cache: false,
        ...
        <div class="row">
            <div class="col-lg-12" >
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">تخفیف رویداد ها</div>
                            ...
                            <form id="form_discount" >
                                <div class="col-sm-6">
                                    <div class="row">
                                        ...
                                        <div class="col-75">
                                            <input type="text" id="code"  class="courseDiscountCode">
                                            <button onclick="checkDuplicateCourseDiscountCode()" style=" background: red; color: #fff;padding: 8px; position: absolute; width: 128px;border-radius: 5px;">بررسی تکراری بودن </button>
                                        </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-75">
                                            <select name="course_id" id="course_id">
                                                @foreach(\App\Course::orderby('id' , 'desc')->get() as $item)
                                ...
                                <div class="row" id="btn">
                                    <a class="btn insert_form" onclick="insert_discount()" href="#">  <i class="fa fa-plus"></i>   ثبت اطلاعات </a>&nbsp;
                                </div>
                            </form>
                                ...
                                <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap text-center" cellspacing="0" width="100%">
                                    ...
                                    @foreach($discount as $data)
                                        <tr class="odd gradeX text-center" id="discount{{$data->id}}">
                                            <td><?php echo $j++?></td>
                                            <td>{{$data->code}}</td>
                                            <td>@isset($data->course){{$data->course->title}}@else - @endisset</td>
                                            <td>{{$data->discount}}</td>
                                            ...
                                            <td>
                                                <a onclick="delete_discount({{$data->id}})" class="btn btn_admin" title="حذف رکورد"><i class="fa fa-trash"></i> </a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </table>
