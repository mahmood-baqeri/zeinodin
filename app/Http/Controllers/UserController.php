...
class UserController extends Controller
{
    ...
    {
        $user = Auth::user();
        // $course = InsertCourse::where(['user_id' => $user->id, 'course_id' => $request->course_id])->first();
        ...
        if ($request->name == "" && $request->email == "" && $request->mobile == "") {
            ...
            if ($course) {
                ...
                $authority = ZarinpalController::purchase($resultPrice , $request->mobile ,$request->email ,'https://zeinodin.org/payment/callback');
                if($authority){
...
//                )->pay()->render();
        ...
        if ($course) {
            ...
            if ($user->id == 39 || $user->id == 4) {
                LicenseController::start($course, $user);
                return redirect()->back();
            }
    ...
    public function payCallback(Request $request)
    {
        ...
        if ($request['Status'] == 'NOK') session()->flash('success' , 'لغو خرید توسط شما');
            ...
            if (!$insertCourse) return redirect()->route('indexPage');

            $verify = ZarinpalController::verify($insertCourse->transactionId, (int)$insertCourse->price);
            if ($verify) {
                ...
                if ($verify['status'] == 'success') {
                    ...
                    if ($insertCourse->course->spotKey)
                        LicenseController::courseLicense($insertCourse, $insertCourse->course->spotKey, $insertCourse->user);
        ...
        return redirect()->action([UserHomeController::class, 'myCourse']);
        ...
        $returnMsg = "";
        try {
            $receipt = Payment::amount((int)$insertCourse->price)->transactionId($insertCourse->authority)->verify();
